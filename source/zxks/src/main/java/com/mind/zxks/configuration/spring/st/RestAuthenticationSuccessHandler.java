...
import com.mind.zxks.base.SystemCode;
...
public class RestAuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        if (null != object) {
            ...
            com.mind.zxks.domain.User user = userService.getUserByUserName(springUser.getUsername());
            if (null != user) {
                UserEventLog userEventLog = new UserEventLog(user.getId(), user.getUserName(), user.getRealName(), new Date());
                userEventLog.setContent(user.getUserName() + " 登录了在线考试管理系统");
                eventPublisher.publishEvent(new UserEvent(userEventLog));
                com.mind.zxks.domain.User newUser = new com.mind.zxks.domain.User();
